<template>
    <div class="container">
        <div v-if="product">
          ...
          <div v-for="(option, index) in product.depositoption_set" :key="index" class="cardlist">
...
const loadProduct = () => {
  axios({
  method: 'get',
  url: `${store.API_URL}/products/deposit-product-options/${route.params.id}/`
})
  .then((response) => {
    product.value = response.data
    product.value.depositoption_set.forEach(element => {
...
const subscribeProduct = (optionId) => {
  axios({
    method: 'post',
    url: `${store.API_URL}/products/subscribe-deposit/${optionId}/`,
    headers: {
